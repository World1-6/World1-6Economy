...
public class VaultManager {
    ...
    private void setupEconomy() {
        if (this.plugin.getServer().getPluginManager().getPlugin("Vault") == null) {
            this.plugin.getServer().getConsoleSender().sendMessage(Translate.chat("Vault was not found?????????????"));
            this.plugin.getServer().getPluginManager().disablePlugin(this.plugin);
        }
        this.plugin.getServer().getServicesManager().register(Economy.class, theCore, this.plugin, ServicePriority.Low);
        this.plugin.getServer().getConsoleSender().sendMessage(Translate.chat("Vault was found!"));
    }
